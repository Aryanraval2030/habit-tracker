...
function Hero({
  ...
  const totalHabitsCount =
    selectedHabits.selected.length + selectedHabits.custom.length;
  const validCompletedDefault =
    completedHabits?.default?.filter((id) =>
      selectedHabits.selected.includes(id),
    ).length || 0;
  const validCompletedCustom =
    completedHabits?.custom?.filter((idx) => idx < selectedHabits.custom.length)
      .length || 0;
  ...
  useEffect(() => {
    ...
    setHabitHistory((prev) => {
      ...
      if (
        current.total === totalHabitsCount &&
  ...
  const calculateStreaks = () => {
    if (!habitHistory || Object.keys(habitHistory).length === 0)
  ...
  const getActivityHeatmap = () => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      if (historyDay && historyDay.total > 0) {
        ...
        if (ratio > 0.66)
          colorClass = "bg-blue-500 shadow-[0_0_5px_rgba(59,130,246,0.5)]";
        else if (ratio > 0.33)
          colorClass = "bg-blue-800 shadow-[0_0_5px_rgba(30,64,175,0.5)]";
